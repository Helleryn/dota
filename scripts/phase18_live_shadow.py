#!/usr/bin/env python3
"""
PHASE 18 — улучшенный live shadow: реальные будущие матчи Dota 2.

Отличия от Phase 17, где было READY=0 / ABSTAIN=38:

1. **Источник расписания — bo3.gg, дисциплина 4.** Phase 16 ошибочно
   использовала `discipline_id=1` (это Counter-Strike 2) и потому мерила
   не ту игру. Здесь дисциплина проверена запросом `/disciplines`.

2. **Сопоставление по ИГРОКАМ, а не по названию команды.** Ник из bo3.gg
   ищется в реестре `proPlayers` OpenDota; берутся только однозначные
   совпадения. Названия команд не используются для связывания вовсе
   (Phase 10: 461 пара одноимённых команд существовала параллельно).

3. **Частичный состав — рабочий режим, а не отказ.** Замер: среди 133
   игроков предстоящих матчей однозначно сопоставлены 88 (66.2%), и
   матчей с полными десятью — ноль. Прогноз строится по известным, число
   известных сохраняется в снимке.

Ограничение, названное прямо: реестр `proPlayers` — снимок «сегодня».
Для ЖИВОГО прогноза (T = сейчас) это допустимо, для исторического
replay — нет, и там он не применяется.

Запуск: python3 scripts/phase18_live_shadow.py [--store]
"""
...
def main(argv=None) -> int:
    ...
    section("PHASE 18 — live shadow на реальных будущих матчах Dota 2")
    matches_raw = _get(
    ...
    section("Прогнозы")
    print(f"  {'матч':>8s} {'через':>7s} {'A/B игроков':>12s} {'статус':>16s} "
          f"{'p_raw':>7s}  причина отказа")
    ...
    for m in matches_raw:
        ...
        if start is None:
            ...
            status = "PARTIAL_DATA" if min(na, nb) < 5 else "READY"
        ...
        print(f"  {mid:>8s} {lead:6.1f}ч {na:>5d}/{nb:<5d} {status:>16s} "
